...
function Logo() {
  return <Link href="/" data-testid="link-logo" className="flex items-center gap-3 focus-ring rounded-lg w-fit">
    <span className="grid size-9 place-items-center rounded-xl bg-[hsl(var(--accent))] text-[hsl(var(--primary))] shadow-sm"><Moon size={18} strokeWidth={2.5} /></span>
    <span className="font-display text-xl tracking-tight">afterlight</span>
  </Link>;
}

export function DemoBadge() {
  return <div data-testid="badge-demo-mode" className="fixed bottom-3 left-1/2 z-50 flex -translate-x-1/2 items-center gap-2 rounded-full border border-[hsl(var(--border))] bg-[hsl(var(--card)/.96)] px-3 py-1.5 text-[10px] font-mono-ui uppercase tracking-wider text-[hsl(var(--muted-foreground))] shadow-lg backdrop-blur-sm">
    <span className="size-1.5 rounded-full bg-[hsl(var(--accent))]" /> Demo Mode — simulated frontend data
  </div>;
}
  ...
  return <div className="min-h-[100dvh] bg-[hsl(var(--background))]">
    ...
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-[255px] flex-col border-r border-[hsl(var(--sidebar-border))] bg-[hsl(var(--sidebar))] px-4 py-5 text-[hsl(var(--sidebar-foreground))] transition-transform md:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <div className="mt-8 border-b border-[hsl(var(--sidebar-border))] pb-5">
        ...
        {venueMenu && <div className="mt-2 rounded-xl border border-[hsl(var(--sidebar-border))] bg-[hsl(var(--sidebar-accent))] p-1.5 text-xs">
          {apiClient.getState().venues.map((venue) => <button key={venue.id} data-testid={`button-venue-${venue.id}`} onClick={() => setVenueMenu(false)} className="flex w-full items-center gap-2 rounded-lg px-2 py-2 text-left hover:bg-[hsl(var(--sidebar))]"><span className="font-mono-ui text-[10px] text-[hsl(var(--accent))]">{venue.initials}</span>{venue.name}</button>)}
      ...
      <div className="space-y-1 border-t border-[hsl(var(--sidebar-border))] pt-4">
        ...
        <button data-testid="button-reset-demo" onClick={() => { apiClient.reset(); window.location.reload(); }} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-[hsl(var(--sidebar-foreground)/.65)] hover:bg-[hsl(var(--sidebar-accent))]"><RotateCcw size={17} />Reset demo state</button>
    ...
    <div className="md:pl-[255px]">
      <header className="sticky top-0 z-30 flex h-[68px] items-center justify-between border-b border-[hsl(var(--border))] bg-[hsl(var(--background)/.92)] px-4 backdrop-blur-md md:px-8">
        ...
        <div className="ml-auto flex items-center gap-3"><ThemeControl /><button data-testid="button-help" className="focus-ring rounded-full p-2 text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--muted))]" aria-label="Help"><HelpCircle size={18} /></button><span className="grid size-8 place-items-center rounded-full bg-[hsl(var(--primary))] text-xs font-bold text-[hsl(var(--primary-foreground))]">{persona === 'venue' ? 'MC' : 'AR'}</span></div>
      </header>
